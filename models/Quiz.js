...
quizSchema.virtual('totalPoints').get(function() {
  ...
  return this.selectedQuestions.reduce((total, q) => total + (q.points || 1), 0);
});

// Virtual for average difficulty
quizSchema.virtual('averageDifficulty').get(function() {
  if (!this.selectedQuestions || this.selectedQuestions.length === 0) return 'medium';
...
quizSchema.methods.validateQuiz = function() {
  ...
  if (this.duration && this.duration > 0 && this.selectedQuestions && this.selectedQuestions.length > 0) {
...
quizSchema.methods.canUserAttempt = function(userAttempts) {
  ...
  const completedAttempts = userQuizAttempt.attempts.filter(attempt => 
    attempt.status === 'completed' || attempt.status === 'timeout' || attempt.status === 'abandoned'
  ).length;
  
  if (completedAttempts >= this.maxAttempts) {
    return { canAttempt: false, reason: 'Maximum attempts reached', attemptsLeft: 0 };
  }
...
quizSchema.methods.getActiveAttempt = function(userAttempts) {
  ...
  return userQuizAttempt.attempts.find(attempt => attempt.status === 'in_progress');
};
