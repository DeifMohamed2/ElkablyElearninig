...
const ContentItemSchema = new mongoose.Schema({
  type: {
    type: String,
    enum: ['video', 'pdf', 'homework', 'quiz', 'assignment', 'reading', 'link'],
    required: true,
  },
  title: {
    type: String,
  ...
  content: {
    type: String, // URL or file path (for non-quiz/homework content)
    required: function() {
  ...
  selectedQuestions: {
    ...
    validate: {
      validator: function(questions) {
        return !['quiz', 'homework'].includes(this.type) || questions.length > 0;
      },
  ...
  unlockConditions: {
    type: String,
    enum: ['immediate', 'previous_completed', 'quiz_passed', 'assignment_submitted'],
    default: 'immediate'
  }
}, { timestamps: true });
