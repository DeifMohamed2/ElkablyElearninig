...
promoCodeSchema.methods.canUserUse = function (userId, userEmail = null) {
  ...
  if (this.restrictToStudents) {
    ...
    const isAllowedById = this.allowedStudents.some(
      (allowedId) => allowedId.toString() === userId.toString(),
    );

    const isAllowedByEmail =
      userEmail &&
      this.allowedStudentEmails.some(
  ...
  const hasUsed = this.usageHistory.some(
...
promoCodeSchema.methods.calculateDiscount = function (orderAmount, items = []) {
  ...
  if (orderAmount < this.minOrderAmount) {
    throw new Error(
      `Minimum order amount of ${this.minOrderAmount} EGP required`,
    );
  }
  ...
  if (this.applicableTo !== 'all') {
    const hasApplicableItems = items.some((item) => {
      if (this.applicableTo === 'bundles' && item.type === 'bundle')
        return true;
      if (this.applicableTo === 'courses' && item.type === 'course')
        return true;
      return false;
    });
    ...
    if (!hasApplicableItems) {
      throw new Error(
        `This promo code is only applicable to ${this.applicableTo}`,
      );
    }
  }
  ...
  if (this.specificItems && this.specificItems.length > 0) {
    const hasSpecificItems = items.some((item) =>
      this.specificItems.some(
        (specificId) => specificId.toString() === item.id.toString(),
      ),
    );

    if (!hasSpecificItems) {
      throw new Error(
        'This promo code is not applicable to the selected items',
      );
    }
  }
...
promoCodeSchema.methods.applyPromoCode = function (
  userId,
  purchaseId,
  orderAmount,
  items = [],
...
promoCodeSchema.statics.findValidPromoCode = async function (
  code,
  userId,
  userEmail = null,
) {
...
promoCodeSchema.statics.generateBulkCodes = async function (
  count,
  prefix = '',
  length = 8,
) {
...
promoCodeSchema.statics.getBulkCollectionStats = async function (
  bulkCollectionId,
) {
