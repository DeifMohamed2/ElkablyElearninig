...
class BunnyCDNService {
  constructor() {
    ...
    this.libraryId = process.env.BUNNY_LIBRARY_ID || '508763';
  ...
  async uploadVideo(videoBuffer, videoId, title = 'Zoom Recording') {
    try {
      ...
      console.error('❌ Error uploading to Bunny CDN:', error.response?.data || error.message);
      ...
      throw new Error(
        `Failed to upload to Bunny CDN: ${error.response?.data?.Message || error.message}`
      );
    }
  }
